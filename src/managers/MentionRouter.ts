...
export class MentionRouter {
  ...
  private async *_sendChildMessage(
    ...
    handler.onDecision = (request, decision) => {
      if (nativeActions && active && !run.controller.signal.aborted && !request.signal.aborted
        && request.panelId === childPanelId && decision === 'deny') {
        nativeActions.denied = true;
      }
    };
    const registration = this._providerManager.setNativeApprovalHandlerForPanel(childPanelId, handler);
    const retire = () => {
      active = false;
