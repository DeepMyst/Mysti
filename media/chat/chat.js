    ...
    "suggestedSkills": ["first-principles", "test-driven", "scope-discipline", "organized", "concise"]
      ...
      if (setupRetryBtn) {
        setupRetryBtn.addEventListener('click', function() {
          // Two clicks used to start two concurrent `_runAutoSetup` runs —
          // `SetupManager.setupProvider` has no in-flight guard, so that is two
          // `npm install -g` racing on the same global prefix. Disable the
          // button, and say what is happening instead of leaving the old error
          // text sitting there.
          //
          // Deliberately does NOT hide `.setup-error`: `#setup-skip-btn` lives
          // INSIDE that pane, so hiding it left a full-screen overlay with no
        ...
        if (optionsList) {
          ...
          payload.options.forEach(function(option) {
            ...
            optionEl.addEventListener('click', function() {
              hideAuthOptionsModal();
              // Plan 28 Phase 7: another request path that bypasses
              // `startProviderSetup`. It ends in `_pollAuthStatus`, whose
              // setupComplete/setupFailed would otherwise be swallowed by a set
      ...
      function handleCheckpointCreated(payload) {
        ...
        var messageEl = Array.from(messagesEl.querySelectorAll('.message')).find(function(message) {
          return message.dataset.id === payload.messageId;
        });
        var btn = messageEl && messageEl.querySelector('.message-rewind-btn');
      ...
      const messageRenderer = window.MystiMessageRenderer.create({
        document,
      ...
      function addMessage(msg) {
        return messageRenderer.appendMessage(messagesEl, msg);
      }
      ...
      function handleToolResult(toolCall) { toolCards.result(toolCall); }
      function augmentToolResult(result) {
        var toolCall = result.toolCall;
        var toolEl = result.element;
        var toolName = result.name;
        var toolInput = result.input;
