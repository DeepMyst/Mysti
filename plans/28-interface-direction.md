...
| **Agents/setup** | **NOT DONE, and smaller than written.** The install-failure remediation the plan asks for is already built — `provider.suggestedFix` renders with a copyable `sudo` block for the permission category. What remains is only that the wizard is a modal overlay rather than a dock. |
