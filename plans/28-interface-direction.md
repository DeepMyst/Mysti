...
| **Approval card** | **DONE (2 of 3).** Effects are now listed before the buttons — writes, deletes, runs a command, reaches the network, delegates, remote-authored input — derived from `actionType` and the gate's own details, never from the command's prose. The timer says which way the clock falls: `auto-denies` / `auto-ACCEPTS` / `Mysti decides`, and a `forceInteractive` or `remoteOrigin` card says auto-denies regardless of the timeout setting. **The blast radius was already built** — `alwaysAllowLabel` (Plan 27 §25) mirrors `bashGrantToken` exactly, including refusing to promise anything for a compound command; the draft of this phase added a second copy and it was deleted as a drift hazard. **NOT done: naming the agent that is asking** — `PermissionRequest` carries no provider, and on a delegated run the active agent is the wrong answer. Adding it truthfully is extension-side work. |
| **Light theme + wide layout** | **Wide layout DONE.** One work area wraps the transcript and both docks: narrow stacks and a dock swaps in, wide (≥900px) becomes a row and the dock sits beside the conversation — asserted by measuring both boxes in a real browser at 420px and at 1280px. **Light theme not separately verified**: every new surface resolves through the Plan 06 token layer, so it should follow, but "should" is not a test. |
| **Context** | **DONE (the missing half).** The per-item activate/deactivate switch already shipped in Plan 07; what was missing was the cost. Every row now carries its own approximate token count, struck through when the row is off, and the aggregate reuses the same helper instead of rounding twice. |
| **Recovery** | **DONE (the missing half).** Categorised provider-error badges already shipped. What did not exist anywhere was **stall detection**: nothing distinguished a long think from a dead process. After 90s with no backend traffic a card says how long, what the run last reported, that a long think looks the same, and that nothing has been cancelled — then offers keep-waiting, stop, or hand-off to another agent down the existing mention path. Auth-expiry and policy-denial cards are **not** built. |
| **Personas & skills** | **DONE (the missing half).** `AgentMetadata` has carried `trusted` and `contentWarnings` since Plan 20 — and `_mapAgentLists` passed them to ROLES but dropped them for personas and skills, so the panel could not tell you whether a skill's text is concatenated into the system prompt or fenced as untrusted data. Both now travel, and each row carries its origin: silent for an unchanged bundled file, `yours` / `this repo` / `imported · N findings` for anything else, and a loud **changed since it shipped** for a bundled file that no longer matches the compiled-in manifest. Worded as provenance rather than a verdict, because the Tier-1 flag is measured at load and an external writer can leave it stale. |
| **Agents/setup** | **PARTLY DONE, and smaller than written.** The install-failure remediation the plan asks for was already built — `provider.suggestedFix` renders with a copyable `sudo` block for the permission category. What the design actually objected to was that the first-run screens TRAP you: both are `position: fixed` full-screen at z-index 100000 and a Skip *button* was the only way out of either — the exact D-1 shape this codebase already shipped once, where the single exit was CSP-dead. **Escape now leaves both**, by clicking the existing skip control so the dismissal persists through the same path (`dontShowAgain: true`) rather than a second copy of that decision. Converting the wizard wholesale into a dock is NOT done: it carries its own state machine (`renderWizard`, provider cards, install flows) and restructuring it unverified by an F5 pass is disproportionate to the remaining complaint. |
| **Connections/MCP** | **ALREADY SHIPPED, and better placed than this plan drew it.** Plan 23 Gate 5 puts tool-description drift on the APPROVAL CARD — "THIS TOOL CHANGED since you last approved it", with previous and current text — rather than in a Connections panel the user would have to go and look at. The card is where the decision is actually made, so nothing was added here. |
| **Plan & progress** | **NOT BUILT — the design overreached.** The canvas said the plan and the live checklist should be the same list. They are produced by different mechanisms at different times: plan options come from `PlanOptionManager` extracting approaches from an answer, while the todo list is the agent's own `TodoWrite` output. Linking them would fabricate a relationship that does not exist, and a checklist that claims to be the plan while the agent tracks something else is worse than two honest surfaces. |
| **Context & cost** | **NOT BUILT.** The breakdown the canvas shows (system / rules / files / conversation) needs per-category token accounting that nothing computes — `usage` carries only `input_tokens` and `cache_read_input_tokens`. Building it means instrumenting prompt assembly, which is real work rather than a UI pass. The cold-resume warning is the tractable half and remains open. |
| **Agent asks you** | **NOT CHANGED, deliberately.** The canvas said the timer should name *which* option it will pick. It cannot: on a semi-autonomous question a model answers, and on a permission card the `SafetyClassifier` decides — neither picks "option 1". The existing text ("AI will answer if no response") is already the true statement, and encoding a false specific would be worse than leaving it. |
