...
| **Agents/setup · Plan & progress · Personas & skills · Context & cost · Connections/MCP** | **NOT DONE.** Five panel redesigns. Note that each is smaller than the plan implies — the audit found the substrate already shipped in most cases (session grants, MCP pin drift, compaction status, the savings ledger, commit-pinned skill imports). What is missing is surfacing, not machinery. |
