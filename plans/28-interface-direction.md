...
| **Plan & progress · Context & cost · Connections/MCP** | **NOT DONE.** Three surfaces. In each case the audit found the machinery already shipped (compaction status and the savings ledger; `McpToolPins` drift detection) and only the surfacing missing. The one genuinely new piece is the cost BREAKDOWN, which needs per-category token accounting the extension does not compute today. |
