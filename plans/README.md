...
> - **Next:** Plan 03 Phase 3c **Step 2** (convert `media/chat/chat.js` to TS modules under `src/webview-src/chat/` with esbuild + lazy chunks) — **gated** on an interactive F5 smoke test (chat renders/streams end-to-end) + runtime baseline capture, since Step 1 changed how the webview loads. After that: Plan 03 Phases 4–8, or pivot to Plan 01 / 04 / 05.
