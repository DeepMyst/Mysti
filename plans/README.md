...
> - **Plan 04 Phase 3 (next, partly gated):** per-CLI MCP-config wiring — write the DeepMyst MCP endpoint + `dm_` Bearer into every MCP-capable backend's config (Claude `.mcp.json`, Codex `config.toml`, Gemini/Qwen settings, Cline, OpenCode). Needs the DeepMyst-side confirmations: (a) the agents/connections **list API** (to pick the agent slug — user chose to wait for it), (b) **MCP streamable-HTTP transport compat** verified against Claude Code, (c) **prod URLs**. The config-writing itself is format-verifiable now with a slug setting as a stopgap. **Phase 4:** in-chat connect flow (webview).
> - **Next (ungated):** Plan 01 Phase 3b (spawn discovery adapters) or Plan 00 Batch 2 item 4 (provider session correctness). Webview-touching work (Plan 03 Phase 4, Plan 01 Phases 4-5, Plan 02 Phases 4-8, Plan 05 canvas, perf Step 2, canvas F-15 cleanup) pairs with the F5 pass. Plan 04 Phase 3 pairs with the DeepMyst-side confirmations above.
>
