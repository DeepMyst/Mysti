...
> - **Plan 01 Phase 3 (partial) complete.** Live-discovery framework landed: `ICliProvider.discoverModels?()`, the registry `refresh()`/`refreshAll()` engine (timeout race, TTL classes, stale-while-revalidate, in-flight dedup), and staggered post-activation warm-up gated by `mysti.models.autoRefresh` (off the activation critical path). Adapters implemented for the local model servers — **Ollama** (`/api/tags`) and **LocalAI** (`/v1/models`) — where the installed set genuinely varies. Providers without `discoverModels` stay curated-only (behavior-safe). Suite green (+8 tests). *(The framework workflow stalled before the adapter stage; framework + local adapters built/verified by hand.)*
> - **Plan 01 Phase 3b (remaining ungated):** CLI-spawn adapters — `cursor-agent models`, `openclaw models list --json`, `opencode models`, `copilot --help` choices, `codex debug models` — plus Claude `/v1/models` (API-key-gated) and Qwen `~/.qwen/settings.json`. All extension-side; each is a self-contained `discoverModels` method.
> - **Plan 04 PIVOTED to DeepMyst-brokered (user direction) — Phases 1-2 done.** User authenticates with DeepMyst (Clerk) on the web; Mysti holds only a `dm_` API key (SecretStorage); DeepMyst keeps all third-party connection credentials and brokers them via its MCP endpoint (`/api/v1/mcp/{slug}`). **Phase 1** (auth foundation: `DeepMystClient` + `DeepMystAuthManager` + commands + UriHandler + settings, 21 tests) and **Phase 2** (Connections panel: `mysti.openConnections`, auth status, agents/connections lists, manage links — `media/connections/`, +5 tests) are committed; suite 986/986. Revised architecture + DeepMyst-side requirements are in `04-connections-and-agent-management.md`.
> - **Plan 04 Phases 3 + 3b done.** Per-CLI MCP-config wiring shipped via `McpConfigManager`: writes the DeepMyst MCP endpoint + `dm_` Bearer into all six MCP-capable backends — Claude `.mcp.json` (workspace, `type:http`), Gemini/Qwen `settings.json` (`httpUrl`), OpenCode `opencode.json` (`mcp`/`type:remote`), Cline globalStorage `cline_mcp_settings.json` (`type:streamableHttp`), and Codex `config.toml` (text-based TOML table management preserving user comments/settings). Idempotent, only touches its own `deepmyst-*` entries, errors rather than corrupting unparseable configs. Applied on sign-in / agent-enable, removed on sign-out. +15 tests (homeDir + globalStorage parent constructor-injected for hermetic temp-dir runs). The DeepMyst `/connect/vscode` auto-link page + `dm_` key minting landed in the DeepMyst repo (PR #493 → main, Render auto-deploy).
> - **Plan 04 Phase 4 done — in-chat connect flow.** When signed into DeepMyst, the chat injects a system-prompt convention teaching every agent to emit `<<<MYSTI_CONNECT:slug>>>` when the user wants to link an external service. `ChatViewProvider` detects the marker in the same accumulated-text pass as `detectMarkers`/`_detectVisualTestTrigger`, strips it from live + persisted text, and posts a `connectionRequired`/`connectionAlready` card; the webview renders a `Link <service>` button that opens `${webUrl}/connections?connect=<slug>` (or the Connections panel to sign in first). Already-linked services are suppressed via a 60s-TTL connections cache; literal placeholder tokens (`slug`/`service`) are ignored to avoid example-echo false positives. `setDeepMystAuth` is wired post-construction in `extension.ts`. Suite **1001/1001**, tsc + provider-literal guard + webpack clean. *Deferred to pair with Plan 03 Phase 4.2:* bounding all three hot-path scans behind a shared `scannedUpTo`+overlap helper (today the connect scan matches the existing two un-bounded full-text scans). *Deferred enhancements (plan §Phase 3):* richer nonce/label/reason marker metadata and the failed-`mcp__`-tool detection lane B.
> - **Next (ungated):** Plan 01 Phase 3b (spawn discovery adapters) or Plan 00 Batch 2 item 4 (provider session correctness). Webview-touching work (Plan 03 Phase 4, Plan 01 Phases 4-5, Plan 02 Phases 4-8, Plan 05 canvas, perf Step 2, canvas F-15 cleanup) pairs with the F5 pass. Remaining Plan 04: Phase 3b live CLI↔DeepMyst MCP handshake verification (needs a real signed-in environment), and the agents-track Phases 4-6 (AgentLoader hardening, agents/skills UI, skills discovery).
>
> ⚠️ **Infra note (2026-06-13):** the workflow sub-agent runtime degraded badly this session — repeated transient "model unavailable" deaths and one 3-hour stall. Each time the agents' file writes flushed to disk and were finished/verified by hand. Prefer hand-execution or small single-agent tasks over large multi-stage workflows until the runtime stabilizes.
