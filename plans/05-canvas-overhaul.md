...
| `canvas-image` | image generation / reference edits | DeepMyst-brokered | `ImageGenerationService` + key |
| `canvas-video` | Sora/Veo clips | DeepMyst-brokered | `VideoGenerationService` + key |
...
1. **create `src/managers/CanvasCapabilityRegistry.ts`:** `slug → { enabled, source }` computed from `DeepMystAuthManager` connection status (reuse plan 04's connections cache), `CanvasSecrets` (local keys), and `mysti.canvas.capabilities.*` settings. Slugs: `canvas-screens`, `canvas-image`, `canvas-video`, `canvas-code`, `figma`, `canva`, `claude-design`. Fires a change event so the prompt bar + tool descriptions re-gate live.
...
3. **route generation through the registry: modify `CanvasManager`/`CanvasToolServer`/prompt-bar handlers:** resolve the capability first — `source==='deepmyst'` → `McpClient` call; `source==='local'` → bundled service (`StitchService`/`ImageGenerationService`/`VideoGenerationService` with `CanvasSecrets` key); `off` → hide the tool/command and emit a `<<<MYSTI_CONNECT:slug>>>` connect card. Results land via `add_asset`/`insert_page` as before, so provenance + staging are unchanged.
...
10. **Brokering boundary for first-party generators** (§9 / Phase 6): DeepMyst connections are Composio/Smithery-backed, which cover Figma/Canva natively but not Mysti's bespoke Stitch/image/video pipelines. Decide with the DeepMyst team whether those get first-party broker endpoints (`/api/v1/mcp/canvas-image` etc., fully keyless) or stay **local-key only** while still appearing in the same optional-capability registry/UI. Until decided, Phase 6 ships local-key fallback for `canvas-screens`/`canvas-image`/`canvas-video` and broker-only for `figma`/`canva`.
