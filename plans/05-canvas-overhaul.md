...
| `StitchService` | **Keep as the local backend of the optional `canvas-screens` capability** | Stitch screens become `html`-mode pages; Stitch is now one *optional, registered* generator reached via a **direct** Stitch MCP (Google's HTTP MCP / community `stitch-mcp`; not a Composio toolkit) or the bundled `@google/stitch-sdk` — not a hardwired pipeline. |
...
| `canvas-screens` | Stitch-style screen generation → `html` pages | **direct Stitch MCP** (Google's HTTP MCP / community `stitch-mcp`) — *not* a Composio toolkit, so not hub-brokered unless Smithery-listed | `StitchService` (`@google/stitch-sdk`) + key |
...
- **Stitch has MCP servers but is not a Composio toolkit.** Google shipped a cloud/HTTP Stitch MCP (Antigravity codelab) and there are robust community servers (`Kargatharaakash/stitch-mcp` zero-config, `davideast/stitch-mcp` CLI, `oogleyskr/stitch-mcp-server` 25 tools: `get_screen_code`/`get_screen_image`/`build_site`). So `canvas-screens` rides the hub only if a Stitch server is Smithery-listed; otherwise it registers **directly** (Phase 2.3 session-linker mechanism). The bundled `@google/stitch-sdk` stays the local fallback.
...
4. **external / source tools by verified availability (§9):**
   ...
   - **`canvas-screens` (Stitch)** is **not** on Composio → register **directly** via the Phase 2.3 session-linker mechanism (Google's HTTP Stitch MCP, or a community `stitch-mcp` server), or fall back to the bundled `@google/stitch-sdk`. Promote to a hub connection only if/when a Stitch server is Smithery-listed.
...
10. **Brokering boundary — mostly resolved by the 2026-06-14 research (§9):** fal (`fal_ai`), Figma, and Canva are all **Composio toolkits**, so they ride the DeepMyst hub with no bespoke endpoint — that covers image/video/media-edit and the external design tools. The one open item is **Stitch**, which is *not* a Composio toolkit: (a) keep it a **direct** MCP (Google's HTTP Stitch MCP / community server, registered via Phase 2.3), or (b) have DeepMyst add it to the hub via Smithery so it's brokered like the rest. Decide with the DeepMyst team; until then Phase 6 ships Stitch direct (with the `@google/stitch-sdk` local fallback). Claude Design is settled: no MCP → consume via the `canva` connection.
