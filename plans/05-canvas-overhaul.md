...
| 24 | Image tool suite w/ provenance (parent/crop/mask lineage, cost) | `generate` + reference-image edits exist; no provenance, no bg-removal/recolor/crop tools | **Adapt** — Phase 4 (provenance + `generate_visual` tool; editing primitives partial) |
...
| `ImageGenerationService` / `VideoGenerationService` | **Keep** (fix F-5/F-9/F-10/F-25) | Direct analogs of DeepMyst's gpt-image-1/fal.ai layer; become `generate_visual`/`generate_video` tools. |
| `CodeGenerationService` | **Keep, narrowed** | Component-from-design stays as the "export to code" path; F-7 fixed; no longer the primary rendering route. |
| `StitchService` | **Keep as one generator among several** | Stitch screens become `html`-mode pages; Stitch stops being the only structured pipeline. |
...
- **Primary — local MCP tool server** (`mysti-canvas`): a stdio MCP server bundled with the extension, registered into canvas-linked chat sessions per CLI. **Session-scoped registration is owned by this plan (Phase 2.3)** — plan 04's `IMcpConfigAdapter` only writes persistent global/workspace config files and defines no session/temp-file/CLI-arg lane, so there is nothing there to reuse for per-session scoping. Per-CLI reality: Claude Code is the only CLI with true per-invocation config (`--mcp-config <tempfile>` appended in `buildCliArgs`); Gemini/Qwen (`settings.json`) and Codex (`config.toml`) support only persistent config, so canvas registration there is a marker-tagged persistent entry written on link and removed on unlink/panel dispose (details and cleanup rules in Phase 2.3). Tools (~20, each description prefixed `READ-ONLY (...)` or `WRITE (stages an edit)` per DeepMyst's contract): `list_pages`, `read_page`, `insert_page`, `edit_page`, `write_page_jsx`, `delete_page`, `reorder_pages`, `set_theme`, `set_format`, `edit_element`, `page_coordinates` (anchor math), `generate_visual`, `generate_video`, `list_assets`, `analyze_visual`, `render_page_preview`, `validate_page`, `repair_page`, `export_artifact`.
...
4. **Where does the MCP tool server get provider/LLM access for `repair_page`/`analyze_visual`?** In-process handlers can reach `ProviderManager`/services directly only if the server runs in the extension host (stdio bridge), not as a spawned child. Decide bridge architecture in Phase 2.2 (extension-host-hosted stdio is the current lean).
...
6. **Stitch's long-term role** once `write_page_jsx` exists: keep as a high-quality screen generator (`screens` kind) feeding `html` pages, or sunset to reduce the service surface? Revisit after Phase 3 quality comparison.
