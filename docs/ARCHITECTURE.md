...
| Message rendering | `media/chat/messageRenderer.js`, `media/chat/toolCards.js` | Restored messages replay thinking/text/tools through the same rendering ports used live. Tool cards own identities and updates without selector interpolation; message IDs and attachment metadata enter the DOM as data. |
...
| Coordinator tools | `CoordinatorToolDispatcher`, `CoordinatorRunBudget`, `CoordinatorLocalExecGate` | One run owns native/text dispatch, bounded read batches, separate budgets, ordered replay, memory/connect dedupe and tool telemetry. Local execution policy receives approval ports; host effects retain their permission and checkpoint gates. Delegation and higher-level orchestration remain in the host. |
