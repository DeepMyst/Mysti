...
| Message rendering | `media/chat/messageRenderer.js`, `media/chat/toolCards.js` | Restored messages replay thinking/text/tools through the same rendering ports used live. Tool cards own identities and updates without selector interpolation, and close live intake at terminal events; message IDs and attachment metadata enter the DOM as data. |
...
| Coordinator tools | `CoordinatorToolDispatcher`, `CoordinatorRunBudget`, `CoordinatorLocalExecGate` | One run owns native/text dispatch, bounded read batches, separate budgets, ordered replay, memory/connect dedupe and tool telemetry. Local execution policy receives approval ports; host effects retain their permission and checkpoint gates. |
| Coordinator delegation and orchestration | `CoordinatorDelegationRunner`, `CoordinatorRunOrchestrator` | Run-local backend/tier/fold state, shared delegation/review accounting, effect-aware reroute, truthful verification, serial turn/tool/delegation handoffs and final-answer rescue. Host adapters retain actual permission/process identities, UI composition and persistence. A run-wide local-effect abort signal survives individual model streams and scopes Stop to that run. |
| Native approvals | `NativeApprovalRequests`, `AcpApproval`, `NativeApprovalPolicy`, `ProviderManager` | Blocking provider requests, authoritative policy, turn-scoped host registrations and exactly one reply to the issuing transport. Native approval capability is explicit; a streamed tool event does not imply support. Pool terminal results carry monotonic possible-side-effect evidence even when an approval preceded a lost execution notification. |
...
| Canvas host sessions | `CanvasTurnJobs`, `CanvasMcpSession` | Streaming turns own their liveness jobs. A design session owns its MCP server and temporary chat registration; replacement/close unlinks immediately and only the latest switch may publish an endpoint after asynchronous startup/shutdown. |
