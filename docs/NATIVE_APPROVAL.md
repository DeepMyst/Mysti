...
- Continue uses `--readonly` in `edit-automatically + ask-permission`. Copilot public turns now require the verified ACP transport; legacy plain-text execution has no fallback.
...
6. **Read-only selection is not uniform.** Cursor only omits `--force` despite installed explicit read-only `--mode plan/ask`. The OpenCode ACP bridge now removes mutating tools from its fixed native agent in restricted modes. Cursor still needs its own transport and policy correction.
