...
- Hermes/Kimi reject every restricted tier before discovery or spawn (2026-09-22 source review): Hermes v2026.9.21 requests permission only for its dangerous-command denylist and `write_file`/`patch`, and Kimi Code 2.0.2 auto-approves in-repository Write/Edit, FetchURL, Agent, AgentSwarm and Skill; both honour inherited yolo/auto-approve user configuration. Their bridge still answers any emitted request (a missing owner denies; only `allow_once` is ever selected), but it cannot see the unasked tools.
...
| Gemini | 0.60.0 (0.58.0 accepted) ACP, admin policy asks for read/write/replace; other tools denied | Only native read_file permitted plus host mutation denial | Yes, file operations; shell/delegation unavailable | [ACP contract](ACP_NATIVE_APPROVAL.md) |
| Cline | 3.0.64 (3.0.61 accepted) ACP with auto-approval disabled and isolated local state | Native plan mode plus host mutation denial | Yes, supported final native tool inputs; environment API key required | [ACP contract](ACP_NATIVE_APPROVAL.md) |
| Copilot | 1.0.83 ACP with manual permission policy and isolated BYOK state; sync shell and per-file edits held for host cards | Native shell/write denial plus host mutation denial | Yes, sync shell and per-file edits; async/detached shells, web and path grants denied | [ACP contract](ACP_NATIVE_APPROVAL.md) |
...
| OpenCode | 1.18.29 ACP with an isolated fixed agent; file/search/fetch requests, and on macOS shell through a Mysti gate plugin | Native executable tool map removes mutations and shell plus host denial | Yes, supported file/search/fetch surface and macOS shell; delegation unavailable | [ACP contract](ACP_NATIVE_APPROVAL.md) |
| Qwen Code | 0.24.4 (0.23.0 accepted) ACP with explicit ask rules for its selected native tools | Mutation tools excluded plus host denial | Yes, read/edit/notebook/foreground shell | [ACP contract](ACP_NATIVE_APPROVAL.md) |
| Hermes | Rejected before launch (most tools never request permission) | Rejected before launch | No; only fully unrestricted turns run. Bridge answers emitted requests | `src/providers/hermes/HermesProvider.ts`; `src/providers/base/AcpApproval.ts` |
| Kimi Code | Rejected before launch (workspace writes, fetch, subagents auto-approved natively) | Rejected before launch | No; only fully unrestricted turns run. Bridge answers emitted requests | `src/providers/kimi/KimiCodeProvider.ts`; `src/providers/base/AcpApproval.ts` |
...
3. **Zero-argument gap removed.** Direct chat, legacy mentions and the collaborator pool stop approval-required notification-only operations, including `{}` inputs and web requests. They do not open an after-the-fact card. Native request cards still accept valid zero-argument operations and bind the decision to the original request.
